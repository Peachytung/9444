...
The baseline recipe uses seed 9444, 30 epochs, 384 x 320 inputs, batch size 2,
AdamW (`lr=3e-4`, `weight_decay=1e-4`), and dropout 0.2. The deterministic
subset uses 3,000 training images and 500 validation images drawn from the
official training split; the official test set remains held out.
...
```bash
python src/create_splits.py --data-root /path/to/UECFOODPIX/data --output-dir splits --train-size 3000 --val-size 500
python src/smoke_test.py --data-root /path/to/UECFOODPIX/data
python src/train_resnet.py --data-root /path/to/UECFOODPIX/data --epochs 30 --width 384 --height 320 --batch-size 2 --lr 0.0003 --dropout 0.2
python src/train_vgg.py --data-root /path/to/UECFOODPIX/data --epochs 30 --width 384 --height 320 --batch-size 2 --lr 0.0003 --dropout 0.2
```
